...
char version_str[32] = "V.042425";  // month/day/year
...
void updateBlynkSwitches(){ // use for Blynk switches
  if (stay_sw4Value) {
    ...
    delay2s(); // Blynk does not continue to run
    digitalWrite(stayPin, LOW);
    stayArmedLEDValue = 1;
    awayArmedLEDValue = 0;
    }
...
if (panic_sw7Value) {
    strcpy(notification_str, "Panic pressed and held for 5 seconds");
    terminal_output();
    // turn on alarm in panic mode
    digitalWrite(panicPin, HIGH);  // output to FOB
    // longer 3 + 2 press needed as while the panic 3 second delay can be programmed  
    // out of keyfob it resets on power loss to fob - not mentioned in fob manual. 
    delay5s(); // Blynk does not continue to run
    digitalWrite(panicPin, LOW);
    }
terminal.flush();
Blynk.run();
